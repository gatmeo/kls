...
\[
  \lambda(t)=diag(t^n,t^{n-1},\dots ,t).
\]
...
\begin{lemma}[lem:]{}
  ...
  \begin{proof}
    For $x\in BwB/B$, $x=bwB$. Write $b=ut$, we can absorb $t$ on the right as $w^{-1}tw\in T\subseteq B$, $twB=w(w^{-1}tw)B=wB$. Hence we can write $x=uwB$. Since $\lim_{t\rightarrow 0}\lambda(t)u\lambda(t)^{-1}\rightarrow 1$, we have
    \[
      \lim_{t\rightarrow 0}\lambda(t)\cdot uwB=\lim_{t\rightarrow 0}(\lambda(t)u\lambda(t)^{-1})wB=wB.
    \]
  \end{proof}
\end{lemma}
...
\begin{lemma}[lem:]{}
  ...
  \]
  In particular, $X^+_J\simeq \mathhbb{A}^{|J|}$. For $J=[n-1]$, we have $X_{[n-1]}^+=Z_U(N)$.
  \begin{proof}
